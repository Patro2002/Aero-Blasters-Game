...
from objects import Background, Player, Enemy, Bullet, Explosion, Fuel, \
	Powerup, Button, Message, BlinkingText    #to link eith object file witl all class name
...
while running:
	for event in pygame.event.get():
		...
		if event.type == pygame.KEYDOWN and game_page:    #movement using keys of the keyboard
			if event.key == pygame.K_LEFT:    #to move left
				moving_left = True
			if event.key == pygame.K_RIGHT:   #to move right
				moving_right = True
			if event.key == pygame.K_SPACE:    #to shoot
				shoot_bullet() 

		if event.type == pygame.MOUSEBUTTONDOWN:    #movement using mouse
			if home_page:
				home_page = False
				game_page = True
				click_fx.play()
	...
	clock.tick(FPS)    #to call fps speed
