...
class Background():
	def __init__(self, win):
		...
		self.reset()
		self.move = True   #to move image

	def update(self, speed):   #for speed of the bg image
		if self.move:
			self.y1 += speed   
			self.y2 += speed

			if self.y1 >= HEIGHT:  #for scrolling effect of image 1
				self.y1 = -HEIGHT
			if self.y2 >= HEIGHT:  #for scrolling effect of image 2
				self.y2 = -HEIGHT
	...
	def reset(self):
		self.x = 0  
		self.y1 = 0     #for image 1
		self.y2 = -HEIGHT    #for image 2


class Player:
	def __init__(self, x, y):   #x and y represents the initial position of the player

		self.image_list = []
		for i in range(2):
			img = pygame.image.load(f'Assets/player{i+1}.png')  #in f string we ca substitute variables
			img = pygame.transform.scale(img, (100, 86))   #(wid, hei)
	...
	def reset(self, x, y):
		...
		self.rect = self.image.get_rect(center=(x, y))     #initially placing player in center
	...
	def update(self, moving_left, moving_right, explosion_group):
		if self.alive:
			...
			if moving_right and self.rect.x < WIDTH - self.width:   #to be in its limit i.e not going out of image
				self.rect.x += self.speed
